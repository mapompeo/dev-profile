...
interface DemandRow extends DemandEntry {
  inProfile: boolean;
  percentage: number;
}

// Static market demand table (Brazilian job market 2024–2025)
const DEMAND_TABLE: DemandEntry[] = [
...
export class LangDemand {
  topLanguages = input<LanguageUsage[]>([]);

  readonly rows = computed<DemandRow[]>(() => {
    const languages = this.topLanguages();
    if (!languages.length) return [];

    const myLangs = new Map<string, number>(languages.map(l => [l.name, l.percentage ?? 0]));

    // Show langs in my profile first, then relevant market langs I don't have
    const merged = [
      ...DEMAND_TABLE.filter(d => myLangs.has(d.lang)),
      ...DEMAND_TABLE.filter(d => !myLangs.has(d.lang) && d.demand === 'Alta').slice(0, 3),
    ].slice(0, 10);
